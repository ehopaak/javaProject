...
public class CollectionTest2  {
	...
	private static void f10() {
		...
		data.offer(new Fruit("귤2", 2000));
		data.offer(new Fruit("귤1", 4000));
		data.offer(new Fruit("귤4", 1000));
		data.offer(new Fruit("귤3", 3000));
		data.offer(new Fruit("귤5", 7000));
	...
	private static void f9() {
		...
		data.push(new Fruit("귤2", 2000));
		data.push(new Fruit("귤1", 4000));
		data.push(new Fruit("귤4", 1000));
		data.push(new Fruit("귤3", 3000));
		data.push(new Fruit("귤5", 7000));
	...
	private static void f8() {
		//Map : HashMap, TreeMap
		Map<String, Fruit> data = new TreeMap<>(new Comparator<String>() {
		...
		NavigableMap<String,Fruit> data2 = ((TreeMap<String,Fruit>) data).descendingMap();
	...
	private static void f4() {
		TreeSet<Integer> scoreSet = new TreeSet<>();		//tree 자동 sort
		
		scoreSet.add(100);									//set 중복불가
		scoreSet.add(20);
		scoreSet.add(50);
		scoreSet.add(80);
		scoreSet.add(80);
		for(Integer num:scoreSet) {
	...
	//List: 순서있음, 중복가능 ArrayList, LinkdeList, Vector(MultiThread환경에서 동기화지원)
	//Set : 순서없음, 중복불가
	//Map : 키와 값의 쌍
	//HashSet : hashcode(), equals로 동등비교, 동등하면 add불가
	...
	private static void f1() {
		...
		data.add("월1");
		data.add("월4");
		data.add("월5");		//같은값이 있으므로 add되지 않는다.
		data.add("월5");
